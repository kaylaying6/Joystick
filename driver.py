...
joystick_home = [501, 502, False]
last_cmd_click = False
sensitivity = True
...
if run_calibration:
    ...
    print(
        f"Calibration of joystick home is completed."
          f" Received{cali_counter} valid message. "
        f"home_x={joystick_home[0]},home_y={joystick_home[1]}"
    )
...
while time.time() - start_time < 30:
    ...
    # Sensitivity check
    if abs(cmd_x) < sensitivity:
        cmd_x = 0
    if abs(cmd_y) < sensitivity:
        cmd_y = 0
    ...
    # ## Execute the clicking command
    if cmd_click is True and last_cmd_click is False:
        mouse.click(button="left")

    # Update the last cmd click with latest cmd_click
    last_cmd_click = cmd_click